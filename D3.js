...
while (numCrew < starWarsCharacters.length) {
  let charMass = starWarsCharacters[numCrew].mass;
  crewMass = crewMass + charMass;
  numCrew++;
}
...
//   console.log("La massa totale del gruppo ammonta a: ", crewMass);
// } while (numCrew < starWarsCharacters.length);

// while (crewMass > 20000) {
//   crewMass + starWarsCharacters.mass;
//   console.log(crewMass);
// }
...
for (let i = 0; i < starWarsCharacters.length; i++) {
  numCas = Math.floor(Math.random() * 10);
}
console.log(starWarsCharacters[numCas]);
